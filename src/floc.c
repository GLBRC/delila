...
#define reportcounter   1
/*

*/

#define spacefeatures   false
/*
*/
...
Static _TEXT features, flocp, bestfeatures;
/*
*/
...
featuretype *featurelist;
{
  ...
  while (f != NULL) {
/* p2c: floc.p: Note: Eliminated unused assignment statement [338] */
    putc('\n', afile->f);
...
Static Void readparameters(flocp, window, asterisk, maxsequences, RangeFrom,
			   RangeTo)
_TEXT *flocp;
long *window;
boolean *asterisk;
long *maxsequences, *RangeFrom, *RangeTo;
{
  /*
*/
  ...
  if (*flocp->name != '\0') {
    if (flocp->f != NULL)
      flocp->f = freopen(flocp->name, "r", flocp->f);
    else
      flocp->f = fopen(flocp->name, "r");
  } else
    rewind(flocp->f);
  if (flocp->f == NULL)
    _EscIO2(FileNotFound, flocp->name);
  RESETBUF(flocp->f, Char);
...
*/
  ...
  if (ch == 'A')
    ...
    *asterisk = false;
  printf("\nFiltered out features are");

  if (!*asterisk)
    printf(" not");
  printf(" written as comments.\n");

  fscanf(flocp->f, "%ld%*[^\n]", maxsequences);
  getc(flocp->f);

  if (*maxsequences < 0) {
    printf("since maxsequences < 0, all sequences will be processed\n");
    *maxsequences = LONG_MAX;
  }

  fscanf(flocp->f, "%ld%ld%*[^\n]", RangeFrom, RangeTo);
  getc(flocp->f);
  if (*RangeFrom > *RangeTo) {
    printf("RangeFrom must not be larger than RangeTo\n");
    halt();
  }
...
*/
    while (*f != NULL && !found) {
      if ((fabs(filteredfeature2->coordinate - (*f)->coordinate) <= window &&
	   filteredfeature2->Ri < (*f)->Ri) & equalstring(
	    filteredfeature2->nametag,
...
{
  long x;
  definetype *firstd = definitionlist;
  stringDelila *WITH;
  long FORLIM;
...
*/
  ...
  long uniquedefinitions;
} ;

Local Void yell_(LINK)
struct LOC_readDF *LINK;
{
  printf("ERROR in reading definitions and features\n");
}

Local Void dodefinition(LINK)
struct LOC_readDF *LINK;
{
  _TEXT TEMP;

  if (LINK->identifier != 'd')
    return;
  if (*LINK->definitionlist == NULL) {
    *LINK->definitionlist = (definetype *)Malloc(sizeof(definetype));
    cleardefinition(LINK->definitionlist);
    LINK->d = *LINK->definitionlist;
    LINK->dprevious = LINK->d;
  } else {
    LINK->dprevious = LINK->d;
    LINK->d->next = (definetype *)Malloc(sizeof(definetype));
    LINK->d = LINK->d->next;
    cleardefinition(&LINK->d);
  }
  readadefinition(LINK->afile, &LINK->d);
  /*

*/
  fscanf(LINK->afile->f, "%*[^\n]");
  getc(LINK->afile->f);
  LINK->countdefinitions++;
  LINK->uniquedefinitions++;
  LINK->d->number = LINK->uniquedefinitions;


  LINK->dtest = *LINK->definitionlist;
  LINK->done = false;
  while (!LINK->done) {
    if (LINK->dtest == NULL) {
      LINK->done = true;
    ...
    if (equalstring(LINK->dtest->nametag, LINK->d->nametag) &&
	LINK->dtest->number != LINK->d->number) {
      LINK->done = true;
      /*
*/
      printf("WARNING: A duplicate definition was found for definition %ld ",
      ...
      TEMP.f = stdout;
      *TEMP.name = '\0';
      writequotestring(&TEMP, LINK->d->nametag);
      printf(".\n");
      printf("         The first definition (number %ld) will be used.\n",
	     LINK->dtest->number);
...
*/
      ...
      LINK->dprevious->next = NULL;
      Free(LINK->d);
      LINK->done = true;
      /*
*/

      LINK->d = LINK->dprevious;
      /*
...
*/
    ...
    if (!LINK->done)
      LINK->dtest = LINK->dtest->next;
  }
}

Local Void doriblmatrix(LINK)
struct LOC_readDF *LINK;
{
  _TEXT TEMP;

  if (LINK->identifier != 'r')
    return;

  getriblmatrix(LINK->afile, &LINK->matrix);

  locatedefinition(LINK->matrix->riblname, *LINK->definitionlist,
		   &LINK->thedefinition);
  if (LINK->thedefinition == NULL) {
    printf("A definition for Ribl matrix ");
    TEMP.f = stdout;
    *TEMP.name = '\0';
    writequotestring(&TEMP, LINK->matrix->riblname);
    printf(" was not found in the features.\n");
    halt();
  }
  ...
  /*
*/
  if (LINK->thedefinition->min < LINK->matrix->frombase ||
      LINK->thedefinition->max > LINK->matrix->tobase) {
    printf("Definition:\n");
    TEMP.f = stdout;
    *TEMP.name = '\0';
    writeadefinition(&TEMP, LINK->thedefinition);
    printf("\nViolates The From-To Bounds of its Matrix:\n");
    printf("matrix frombase: %ld\n", LINK->matrix->frombase);
    printf("matrix   tobase: %ld\n", LINK->matrix->tobase);
    halt();
  }


  LINK->thedefinition->matrix = LINK->matrix;
  LINK->countribls++;
}



Static Void readDF(afile_, definitionlist_, featurelist, holdover, best,
		   lower)
_TEXT *afile_;
definetype **definitionlist_;
featuretype **featurelist, **holdover;
_TEXT *best, *lower;
{
  /*
...
*/
  ...
  if (*holdover == NULL) {
    if (debugging)
      printf("holdover was nil so start fresh\n");
    firstfeature = true;
    *featurelist = (featuretype *)Malloc(sizeof(featuretype));
    clearfeature(featurelist);
    f = *featurelist;
    *V.definitionlist = NULL;
    ...
    if (debugging)
      printf("holdover was NOT nil so start use it:\n");
    if (debugging)
      printf("holdover->");
    if (debugging) {
      TEMP.f = stdout;
      *TEMP.name = '\0';
      writeafeature(&TEMP, *holdover);
    }
    firstfeature = false;
    *featurelist = *holdover;
    f = *featurelist;
    *holdover = NULL;
    V.d = *V.definitionlist;
  }

  complete = BUFEOF(V.afile->f);
  while (!complete) {
    V.identifier = P_peek(V.afile->f);

    if (V.identifier == 'd') {
      dodefinition(&V);
      writeadefinition(best, V.d);
      writeadefinition(lower, V.d);
    }

    else if (V.identifier == 'r') {
      doriblmatrix(&V);
      putriblmatrix(best, V.d->matrix);
      putriblmatrix(lower, V.d->matrix);
    }
      ...
      if (firstfeature) {
	firstfeature = false;
	readafeature(V.afile, *V.definitionlist, &f);
	fscanf(V.afile->f, "%*[^\n]");
	getc(V.afile->f);
	/*
*/
	...
	newfeature = (featuretype *)Malloc(sizeof(featuretype));
	clearfeature(&newfeature);
	readafeature(V.afile, *V.definitionlist, &newfeature);
	fscanf(V.afile->f, "%*[^\n]");
	getc(V.afile->f);
	/*
*/
	...
	/*
*/
	if (equalstring(newfeature->id, f->id) &
	    equalstring(newfeature->nametag, f->nametag)) {
	  if (debugging)
	    printf("same name as previous feature f\n");
...
*/
	...
	*TEMP.name = '\0';
	writestring(&TEMP, &buffer);
      ...
      if (debugging)
	putchar('\n');
      writestring(best, &buffer);
      putc('\n', best->f);
      writestring(lower, &buffer);
      putc('\n', lower->f);
...
*/
  ...
  pa = *a_;
  *b = (featuretype *)Malloc(sizeof(featuretype));
  pb = *b;
  if (debugging)
    printf("copyfeatures\n");
  while (pa != NULL) {
    if (debugging) {
      TEMP.f = stdout;
      *TEMP.name = '\0';
      writeafeature(&TEMP, pa);
      putchar('\n');
    }
    if (debugging) {
      TEMP.f = stdout;
      *TEMP.name = '\0';
      writeafeature(&TEMP, pb);
      putchar('\n');
    }
    *pb = *pa;
    if (pa->next != NULL) {
      pb->next = (featuretype *)Malloc(sizeof(featuretype));
      pb = pb->next;
    } else
      pb->next = NULL;
    pa = pa->next;
...
Static Void splitfeatures(f, asterisk, sequencenumber, bestfeatures,
			  lowerfeatures, RangeFrom, RangeTo)
featuretype *f;
boolean asterisk;
long sequencenumber;
_TEXT *bestfeatures, *lowerfeatures;
...
*/
  ...
  if (spacefeatures) {
    fprintf(bestfeatures->f, "\n* %ld\n", sequencenumber);
    fprintf(lowerfeatures->f, "\n* %ld\n", sequencenumber);
  }
...
*/
    if (a_->coordinate >= RangeFrom && a_->coordinate <= RangeTo) {
      if (a_->Ri > maxri)
	maxri = a_->Ri;
    }
...
*/
  ...
  while (a_ != NULL) {
    if (a_->coordinate >= RangeFrom && a_->coordinate <= RangeTo)
      ...
      grabit = false;
    if (grabit) {
      writeafeature(bestfeatures, a_);
      putc('\n', bestfeatures->f);
      if (asterisk) {
	fprintf(lowerfeatures->f, "* ");
	writeafeature(lowerfeatures, a_);
	putc('\n', lowerfeatures->f);
      }
      count++;
    } else {
...
Static Void dumpfeatures(f, afile)
featuretype **f;
_TEXT *afile;
{
  featuretype *a_;
...
Static Void themain(features, flocp, bestfeatures, lowerfeatures)
_TEXT *features, *flocp, *bestfeatures, *lowerfeatures;
{
  boolean asterisk;
  /*
*/
  definetype *definitionlist;
  long discardednumber = 0;
  featuretype *f;
  /*
*/
  long filterednumber = 0;
  featuretype *holdover = NULL;
  long sequencenumber = 0;
  long maxsequences, RangeFrom, RangeTo, window;
  /*
*/
  ...
  *TEMP.name = '\0';
  ...
  readparameters(flocp, &window, &asterisk, &maxsequences, &RangeFrom,
		 &RangeTo);
  ...
  while (!BUFEOF(features->f) && sequencenumber < maxsequences) {
    sequencenumber++;
    TEMP.f = stdout;
    *TEMP.name = '\0';
    /*
...
*/
    digitcounter(&TEMP, &sequencenumber, &previoussequencenumber);

    readDF(features, &definitionlist, &f, &holdover, bestfeatures,
	   lowerfeatures);
    /*

*/
    splitfeatures(f, asterisk, sequencenumber, bestfeatures, lowerfeatures,
		  RangeFrom, RangeTo);
    clearfeatures(&f);
  }
...
Char *argv[];
{
  ...
  flocp.f = NULL;
  strcpy(flocp.name, "flocp");
  features.f = NULL;
  strcpy(features.name, "features");
  themain(&features, &flocp, &bestfeatures, &lowerfeatures);
_L1:
  if (features.f != NULL)
    fclose(features.f);
  if (flocp.f != NULL)
