...
Static Void initialize(book, dp, apiece)
_TEXT *book, *dp;
piece **apiece;
{
  brinit(book);
  if (*dp->name != '\0') {
    if (dp->f != NULL)
      dp->f = freopen(dp->name, "w", dp->f);
    else
      dp->f = fopen(dp->name, "w");
  } else {
    if (dp->f != NULL)
      rewind(dp->f);
    else
...
Static Void showinfo(dp, apiece)
_TEXT *dp;
piece *apiece;
{
  long length;
...
/* p2c: todawg.p, line 787: Note:
 * Format for packed-array-of-char will work only if width < length [321] */


  fprintf(dp->f, "%.*s", WITH->length, WITH->letters);
  fprintf(dp->f, " %ld", inttopie(1L, apiece));
  fprintf(dp->f, " %ld\n", inttopie(length, apiece));

}



Static Void showsequence(dp, apiece)
_TEXT *dp;
piece *apiece;
{
  long i, FORLIM;
  ...
  for (i = 1; i <= FORLIM; i++) {
    fputc(basetochar(getbase(i, apiece)), dp->f);
    if (i % lengthofline == 0)
      putc('\n', dp->f);
