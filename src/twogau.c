...
Static _TEXT data, twogaup, color;
...
*/
  ...
  /*
*/
/* p2c: twogau.p: Note: Eliminated unused assignment statement [338] */
  if (D > 2) {
...
Static Void themain(data, twogaup, color_)
_TEXT *data, *twogaup, *color_;
{
...
*/
  ...
  readparameters(twogaup, &D, &n, &steps, &V.partial, &idstring);
  TEMP.f = stdout;
  *TEMP.name = '\0';
...
*/
  ...
  if (idstring == 'a')
    fprintf(V.color->f, "twogau_%4.2f %*.*f %*.*f %d %d 0 1 1 0 1 0 %4.2f\n",
...
Char *argv[];
{
  ...
  twogaup.f = NULL;
  strcpy(twogaup.name, "twogaup");
  data.f = NULL;
  strcpy(data.name, "data");
  themain(&data, &twogaup, &color);
_L1:
  if (data.f != NULL)
    fclose(data.f);
  if (twogaup.f != NULL)
