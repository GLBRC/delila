...
Static _TEXT inst, book, hcp, namebook, namelist, avalues, list, clist,
	     xygraph, colors;
...
Static long mono[(long)t - (long)a + 1];
/*
*/
Static base monoindex, abase;
Static long monototal;
Static double monof, monoH;
Static long xgraph, ygraph;
...
*/
  for (logplace = *linesused - 1; logplace >= 0; logplace--) {
    putc('*', afile->f);
    for (spacecount = 2; spacecount <= spaces; spacecount++)
      putc(' ', afile->f);
...
*/
  ...
  *count = 0;
/* p2c: hc.p: Note: Eliminated unused assignment statement [338] */
  while (!BUFEOF(book->f)) {
    switch (alignmenttype) {
...
Static Void getcolumnvalue(f, column, value)
_TEXT *f;
long column;
double *value;
{
...
*/
  putc('*', list.f);
  for (index = 2; index <= blanks; index++)
    putc(' ', list.f);
  FORLIM = todo;
...
*/
  ...
  *internal.name = '\0';
  ...
  if (*hcp.name != '\0') {
    if (hcp.f != NULL)
      hcp.f = freopen(hcp.name, "r", hcp.f);
    else
      hcp.f = fopen(hcp.name, "r");
  } else
    rewind(hcp.f);
  if (hcp.f == NULL)
    _EscIO2(FileNotFound, hcp.name);
  RESETBUF(hcp.f, Char);
  ...
  if (internal.f == NULL)
    _EscIO2(FileNotFound, internal.name);
  SETUPBUF(internal.f, Char);
  almostcopyfile(&hcp, &internal);

  if (*hcp.name != '\0') {
    if (hcp.f != NULL)
      hcp.f = freopen(hcp.name, "w", hcp.f);
    else
      hcp.f = fopen(hcp.name, "w");
  } else {
    if (hcp.f != NULL)
      rewind(hcp.f);
    else
  ...
  SETUPBUF(hcp.f, Char);
  fprintf(hcp.f,
    "%4.2f        version of hcp that this parameter file is designed for.\n",
    parameterversion);
...
Static Void upgradeparameters(hcp)
_TEXT *hcp;
{
  fscanf(hcp->f, "%lg%*[^\n]", &parameterversion);
  getc(hcp->f);
  ...
  printf("https://alum.mit.edu/www/toms/delila/hc.html\n");
}
...
*/
  ...
  if (!BUFEOF(hcp.f)) {
    fscanf(hcp.f, "%c%c%c%*[^\n]", &displaylevel, &usefullname, &shownumbers);
    getc(hcp.f);
    if (displaylevel == '\n')
      displaylevel = ' ';
    if (usefullname == '\n')
  ...
  if (BUFEOF(hcp.f)) {
    printf("you are missing parameter columntoread etc of hcp!\n");
    halt();
  }
  cn(&V);
  fscanf(hcp.f, "%ld%ld%ld%*[^\n]", &columntoread, &columnwid, &columndec);
  getc(hcp.f);

  if (BUFEOF(hcp.f)) {
    printf("you are missing parameter edgecontrol etc of hcp!\n");
    halt();
  }

  edgecontrol = getc(hcp.f);
  if (edgecontrol == '\n')
    edgecontrol = ' ';
  if (edgecontrol != 'p')
    fscanf(hcp.f, "%lg%lg%lg%lg", &edgeleft, &edgeright, &edgelow, &edgehigh);
  fscanf(hcp.f, "%*[^\n]");

  getc(hcp.f);
  if (BUFEOF(hcp.f)) {
    printf("you are missing parameter fontsize etc of hcp!\n");
    halt();
  }

  colorlistcontrol = getc(hcp.f);
  if (colorlistcontrol == '\n')
    colorlistcontrol = ' ';
  ...
  cn(&V);
  fscanf(hcp.f, "%lg", &deltaXcm);
  cn(&V);
  fscanf(hcp.f, "%lg", &deltaYcm);
  cn(&V);
  fscanf(hcp.f, "%lg%*[^\n]", &scaleimage);
  getc(hcp.f);
...
Char *argv[];
{
  ...
  if (*xygraph.name != '\0') {
    if (xygraph.f != NULL)
      xygraph.f = freopen(xygraph.name, "w", xygraph.f);
    else
      xygraph.f = fopen(xygraph.name, "w");
  } else {
    if (xygraph.f != NULL)
      rewind(xygraph.f);
    else
  ...
  if (xygraph.f == NULL)
    _EscIO2(FileNotFound, xygraph.name);
  SETUPBUF(xygraph.f, Char);

  linenumber = 0;
  if (headercontrol == 'h') {
    fprintf(list.f, "* hc %4.2f, aligned listing of book:\n", version);
    linenumber++;
  ...
  if (headercontrol == 'h') {
    fprintf(clist.f, "(hc %4.2f, aligned listing of book: ) sn", version);
    noheader(&clist);
    clinenumber++;
...
*/
    ...
    column = 0;
    for (monoindex = a;
	 (long)monoindex <= (long)t;
	 monoindex = (base)((long)monoindex + 1))
      mono[(long)monoindex - (long)a] = 0;
    xgraph = 0;
    ygraph = 0;

    fprintf(xygraph.f, "%ld %ld\n", xgraph, ygraph);
    FORLIM = todo;
...
*/
	abase = getbase(index_ + alignedbase, apiece);
	b = basetochar(abase);
	mono[(long)abase - (long)a]++;
	putc(b, list.f);
	fprintf(clist.f, " %c", b);
...
*/
    ...
    for (monoindex = a;
	 (long)monoindex <= (long)t;
	 monoindex = (base)((long)monoindex + 1))
      monototal += mono[(long)monoindex - (long)a];
    for (monoindex = a;
	 (long)monoindex <= (long)t;
	 monoindex = (base)((long)monoindex + 1)) {
      if (mono[(long)monoindex - (long)a] > 0) {
	monof = (double)mono[(long)monoindex - (long)a] / monototal;
	monoH -= monof * log(monof);
      }
    }
    monoH /= log(2.0);
    columnvalue = monoH;
