...
Static _TEXT book, livep, livemarks;
...
Static Void readparameters(livep, p)
_TEXT *livep;
parameters *p;
{
  if (*livep->name != '\0') {
    if (livep->f != NULL)
      livep->f = freopen(livep->name, "r", livep->f);
    else
      livep->f = fopen(livep->name, "r");
  } else
    rewind(livep->f);
  if (livep->f == NULL)
    _EscIO2(FileNotFound, livep->name);
  RESETBUF(livep->f, Char);
  fscanf(livep->f, "%lg%*[^\n]", &p->parameterversion);
  getc(livep->f);
  ...
  if ((P_peek(livep->f) == 'f') | (P_peek(livep->f) == 'l')) {
...
parameters p;
{
  ...
  SETUPBUF(f->f, Char);
  fprintf(f->f, "* live %4.2f, create LIster waVEs\n\n", version);
  writeparameters(f, &p);
...
Static Void markat(livemarks, x, y, waveheight, color)
_TEXT *livemarks;
...
*/
  fprintf(livemarks->f, "U %10.5f %10.5f %10.5f %10.5f %10.5f wavebox\n",
...
long huge_;
{
  ...
  markat(&livemarks, (double)cooexternal, shiftdown + p.wavevertical,
	 p.waveheight, thecolor);
}


Static Void bookloop(thezero, internal, pie, p, huge_)
long thezero, internal;
piece *pie;
parameters p;
long huge_;
...
*/
  ...
  thecolor = setcolor(thezero, internal, p.wavelength, huge_);

  markat(&livemarks, (double)inttopie(internal, pie),
	 shiftdown + p.wavevertical, p.waveheight, thecolor);
}


Static Void themain(book, livep, livemarks)
_TEXT *book, *livep, *livemarks;
{
  ...
  printf("live %4.2f\n", version);
  readparameters(livep, &p);
  makelivemarks(livemarks, p);
  p.firstpiece = true;
...
*/
    while (!BUFEOF(livep->f)) {
      fscanf(livep->f, "%ld%ld%*[^\n]", &p.firstbase, &p.lastbase);
      getc(livep->f);

      huge_ = labs(p.lastbase - p.firstbase) * 2;
      switch (p.basephase) {
  ...
  do {
    ...
    if (pieopen) {
      p.firstbase = 1;
      p.lastbase = piecelength(pie);
      if (!p.firstpiece)
	fprintf(livemarks->f, "p - live: skip to next piece\n\n");
      else
	p.firstpiece = false;
      fprintf(livemarks->f, "* piece %ld\n", number);
      /*
...
*/
	WITH = &pie->key;
	if (p.zerobase >= WITH->piebeg && p.zerobase <= WITH->pieend)
	  thezero = pietoint(p.zerobase, pie);
	else if (WITH->piedir == plus) {
	  if (p.zerobase < WITH->piebeg)
	    thezero = p.zerobase - WITH->piebeg + 1;
	  else
	    thezero = p.zerobase - WITH->pieend - 1;
	} else {
	  if (p.zerobase > WITH->pieend)
	    thezero = WITH->pieend - p.zerobase - 1;
	  else
	    thezero = WITH->piebeg - p.zerobase + 1;
	}
	inthezero = thezero;
	break;
	/*
...
*/
	...
	bookloop(inthezero, internal, pie, p, huge_);
...
Char *argv[];
{
  ...
  if (setjmp(_JL1))
    goto _L1;
  livemarks.f = NULL;
  strcpy(livemarks.name, "livemarks");
  livep.f = NULL;
  strcpy(livep.name, "livep");
  book.f = NULL;
  strcpy(book.name, "book");
  themain(&book, &livep, &livemarks);
_L1:
  if (book.f != NULL)
    fclose(book.f);
  if (livep.f != NULL)
